...
High-level API:
    ...
    img.add_file("System32/config/SYSTEM", "boot/data/SYSTEM")
    img.write("boot/data/disk.raw")
...
class DiskImage:
    ...
    def write_header(self, out_path: str | os.PathLike) -> None:
        ...
        if not hasattr(self, "_mbr_checksum"):
            raise RuntimeError("call write() before write_header()")
        Path(out_path).write_text(
            "/* Generated by tools/mkdisk.py — do not edit by hand. */\n"
            "#ifndef _BOOT_DISK_H_\n"
            "#define _BOOT_DISK_H_\n"
            f"#define BOOT_DISK_SIGNATURE  0x{self.signature:08X}u\n"
            f"#define BOOT_DISK_CHECKSUM   0x{self._mbr_checksum:08X}u\n"
            "#endif\n"
        )
