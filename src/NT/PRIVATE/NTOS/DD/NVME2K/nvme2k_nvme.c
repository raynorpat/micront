...
int NvmeBuildReadWriteCommand(IN PHW_DEVICE_EXTENSION DevExt, IN PSCSI_REQUEST_BLOCK Srb, IN PNVME_COMMAND Cmd, IN USHORT CommandId)
{
    ...
    switch (cdb->CDB10.OperationCode) {
        ...
        case SCSIOP_WRITE16:
            ...
            // Split into two ULONGs - CL 8.50 (NT 3.5 toolchain) ICEs on
            // an 8-deep ULONGLONG shift-and-OR chain in a single expression.
            {
                ULONG lbaHi = ((ULONG)Srb->Cdb[2] << 24) |
                              ((ULONG)Srb->Cdb[3] << 16) |
                              ((ULONG)Srb->Cdb[4] <<  8) |
                              ((ULONG)Srb->Cdb[5]);
                ULONG lbaLo = ((ULONG)Srb->Cdb[6] << 24) |
                              ((ULONG)Srb->Cdb[7] << 16) |
                              ((ULONG)Srb->Cdb[8] <<  8) |
                              ((ULONG)Srb->Cdb[9]);
                lba = ((ULONGLONG)lbaHi << 32) | (ULONGLONG)lbaLo;
            }
    ...
    if ((DevExt->TotalRequests % 10000) == 0) {
        ScsiDebugPrint(0, "nvme2k: Stats - Reqs=%u R=%u W=%u BytesR=%08x%08x BytesW=%08x%08x MaxR=%u MaxW=%u PRP=%u/%u Rejected=%u\n",
                       DevExt->TotalRequests,
                       DevExt->TotalReads,
                       DevExt->TotalWrites,
                       (ULONG)(DevExt->TotalBytesRead >> 32),
                       (ULONG)(DevExt->TotalBytesRead),
                       (ULONG)(DevExt->TotalBytesWritten >> 32),
                       (ULONG)(DevExt->TotalBytesWritten),
                       DevExt->MaxReadSize,
                       DevExt->MaxWriteSize,
