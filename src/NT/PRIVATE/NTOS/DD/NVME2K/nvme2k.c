...
scanloop:
...
#ifdef NVME2K_DBG
    /* slotNumber is NT's packed (func<<5 | dev) - decode for the log
       so non-fn-0 devices don't look like absurd slot indices. */
    ScsiDebugPrint(0, "nvme2k: HwFindAdapter - %d:%d.%d VID=%04X DID=%04X Class=%02X%02X%02X\n",
                   busNumber,
                   slotNumber & 0x1F,
                   (slotNumber >> 5) & 0x07,
    ...
    /* The slot info already printed above shows the class - no extra
       "not an NVMe device" line needed. */
