...
Source: `stuff/docs/prefix.doc` (transcribed via soffice + pandoc + cleanup-md.py).
...
a prefix table package. The **Windows NT** prefix table package is
designed for storing and matching path name prefixes.
...
The programmer can then use the CONTAINING_RECORD macro to associate
the prefix table entry with the local data structures.

Only prefixes that match whole logical parts of a path name are
returned. For example, if a table contains the prefix "\Alpha\Beta"
then a look up on "\Alpha\", "\Alpha\Bet" and "\Alpha\BetaGamma"
will be unsuccessful, but a look up on "\Alpha\Beta" and
"\Alpha\Beta\Gamma" will be successful.
...
`PfxFindPrefix` - Search a prefix table for the longest matching
prefix.

`PfxNextPrefix` - Enumerate all of the prefixes stored in a prefix
table.
...
*PrefixTableEntry* - A pointer to the prefix table entry to use to
denote the prefix

This procedure has a return value of TRUE if the prefix was not already
in the table, and FALSE otherwise.

The prefix table keeps a reference to the input *Prefix* string, so once
a prefix is added the input string must not be changed by the user.
...
*FullString* - A pointer to the string to use as the key in searching
the prefix table

*CaseInsensitive* - Indicates if the prefix look up should be preformed
in a manner that ignores the case (*CaseInsensitive* is TRUE) or expects
an exact, case sensitive, match (*CaseInsensitive* is FALSE)
...
3. Dropped *PrefixLength* from `PfxFindPrefix`, and added
...
1. Changed `PfxFindPrefix` to return a pointer to the table entry if
one is found instead of an OUT parameter.
