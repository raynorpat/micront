    ...
    *(PWCHAR)(((PUCHAR)ConfigurationPath)+ConfigurationString->Length) = (WCHAR)L'\0';
    ...
    while ((*CurBindValue != 0) && (*CurExportValue != 0)) {
        ...
        if (IsAMiniport) {
            ...
            DbgPrint("NDIS: calling MiniportInitialize Miniport=%p\n", Miniport);
            ...
            DbgPrint("NDIS: MiniportInitialize returned 0x%08x Miniport=%p\n",
                     MiniportInitializeStatus, Miniport);

            DbgPrint("NDIS: A1 InInitialize=FALSE Miniport=%p\n", Miniport);
            Miniport->InInitialize = FALSE;
            DbgPrint("NDIS: A2 CHECK_FOR_NORMAL_INTERRUPTS Interrupt=%p\n",
                     Miniport->Interrupt);
            CHECK_FOR_NORMAL_INTERRUPTS(Miniport);
            DbgPrint("NDIS: A3 post-CHECK_FOR_NORMAL_INTERRUPTS\n");
            ...
            if (MiniportInitializeStatus == NDIS_STATUS_SUCCESS) {
                ...
                //
                DbgPrint("NDIS: post-init query OID_GEN_MAXIMUM_LOOKAHEAD\n");
                ...
                KeLowerIrql(OldIrql);
                UNLOCK_MINIPORT(Miniport, LocalLock);
                DbgPrint("NDIS: OID_GEN_MAXIMUM_LOOKAHEAD -> status=0x%08x value=%u\n",
                         NdisStatus, MaximumLongAddresses);
