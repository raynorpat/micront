...
Source: `stuff/docs/suspend.doc` (transcribed via soffice + pandoc + cleanup-md.py).
...
is not suspended, then a normal kernel `APC` is queued to the thread
which will cause it to wait on its builtin semaphore.
...
suspended and then resumed, but has never actually received the `APC`
and suspended itself. Since the target thread has never actually
suspended itself, the builtin semaphore count is decremented to indicate
that the thread should suspend rather than resume.
...
>
> 2. before the thread can respond to the suspend `APC`, it is
> resumed which causes the suspend count to be decremented to zero and
> the builtin suspend semaphore to be incremented to one
>
> 3. the thread receives the suspend `APC`, but before it can wait on
> the builtin semaphore it is interrupted to deliver a special kernel
> `APC`
>
> 4. the special kernel `APC` code page faults and waits on the page
> to be brought into memory
>
> 5. the target thread is again suspended which causes its suspend
> count to be incremented and its builtin suspend `APC` to be queued
>
> 6. the thread is resumed before it has finished processing the
> special kernel `APC` which causes the suspend count to be
> decremented to zero and the builtin semaphore to be incremented to two

No additional nesting can occur since further attempts to queue the
`APC` will fail which cause the semaphore count to be decremented.
Thus the maximum count does not need to be greater than two.
