...
Routine Description:
    ...
    Stubbed out.  MicroNT runs pagefile-less by design: physical RAM
    is sized to the workload and the commit limit is RAM-only.  This
    is the only routine that registers a paging file with the Memory
    Manager, so stubbing it makes "no paging file, ever" an enforced
    kernel invariant rather than a boot-time convention -- and drops a
    privileged code path (open a file, build MDLs, wire it into the
    modified page writer) that no MicroNT workload exercises.  The
    modified page writer's pagefile half is consequently unreachable:
    the paging-file list stays permanently empty.

    The original implementation opened the named file, test-wrote a
    page, and built the structures to use it as a paging file; see
    the project history if paging support is ever reinstated.

Arguments:

    PageFileName, MinimumSize, MaximumSize, Priority - ignored.

Return Value:

    Always STATUS_NOT_IMPLEMENTED.
