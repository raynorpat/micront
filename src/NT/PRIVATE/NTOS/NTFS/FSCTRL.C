...

{
    ...
    //
    // MicroNT: early OEM-signature check.  A real NTFS volume's boot
    // sector has the bytes "NTFS    " at offset 3 (8 bytes, space-
    // padded).  Without this check, NtfsMountVolume happily charges
    // through Vcb/Scb/cache-manager setup before NtfsIsBootSectorNtfs
    // (~250 lines down) finally validates the OEM ID; on a FAT16
    // volume that intermediate machinery operates on garbage and
    // crashes — INACCESSIBLE_BOOT_DEVICE before fastfat ever gets a
    // chance at the volume.
    //
    // We do a raw IRP-level read of sector 0 directly against the
    // underlying device, before touching any NTFS-side state.
    // Returning STATUS_UNRECOGNIZED_VOLUME early lets the I/O manager
    // try the next FS recognizer (fastfat).  This is the standard
    // NT FS-recognizer pattern; FsRec uses the same OEM-bytes check
    // before loading the heavy FS driver at all.
    //
    {
        PDEVICE_OBJECT  TargetDevice;
        PIRP            ProbeIrp;
        KEVENT          ProbeEvent;
        IO_STATUS_BLOCK ProbeIoStatus;
        LARGE_INTEGER   ProbeOffset;
        UCHAR          *ProbeBuf;
        ULONG           SectorBytes = 512;     /* OEM ID is in first 16 B */

        TargetDevice = IoGetCurrentIrpStackLocation(Irp)->
                            Parameters.MountVolume.DeviceObject;

        ProbeBuf = ExAllocatePool( NonPagedPoolCacheAligned, SectorBytes );
        if (ProbeBuf != NULL) {

            KeInitializeEvent( &ProbeEvent, NotificationEvent, FALSE );
            ProbeOffset.QuadPart = 0;

            ProbeIrp = IoBuildSynchronousFsdRequest(
                            IRP_MJ_READ,
                            TargetDevice,
                            ProbeBuf,
                            SectorBytes,
            ...
            if (ProbeIrp != NULL) {
                ...
                Status = IoCallDriver( TargetDevice, ProbeIrp );
                if (Status == STATUS_PENDING) {
                    KeWaitForSingleObject( &ProbeEvent, Executive,
                                           KernelMode, FALSE, NULL );
                    Status = ProbeIoStatus.Status;
                }

                if (NT_SUCCESS( Status )) {

                    BOOLEAN IsNtfs =
                        (ProbeBuf[3] == 'N') && (ProbeBuf[4] == 'T') &&
                        (ProbeBuf[5] == 'F') && (ProbeBuf[6] == 'S') &&
                        (ProbeBuf[7] == ' ') && (ProbeBuf[8] == ' ') &&
                        (ProbeBuf[9] == ' ') && (ProbeBuf[10] == ' ');

                    ExFreePool( ProbeBuf );

                    if (!IsNtfs) {
                        NtfsCompleteRequest( &IrpContext, &Irp,
                                             STATUS_UNRECOGNIZED_VOLUME );
                        return STATUS_UNRECOGNIZED_VOLUME;
                    }
                    /* OEM bytes match -- fall through to full mount path. */

                } else {
                    /* Couldn't read sector 0 at all -- not our problem. */
                    ExFreePool( ProbeBuf );
                    NtfsCompleteRequest( &IrpContext, &Irp,
                                         STATUS_UNRECOGNIZED_VOLUME );
                    return STATUS_UNRECOGNIZED_VOLUME;
                ...
                ExFreePool( ProbeBuf );
                /* IoBuildSynchronousFsdRequest failure: fall through and
                 * let the canonical mount path try -- worst case it'll
                 * fail the same way and we surface that error. */
            }
        }
        /* Allocation failure: same fall-through. */
    }
