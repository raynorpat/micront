...
Source: `stuff/docs/execsupp.doc` (transcribed via soffice + pandoc + cleanup-md.py).
...
documented elsewhere in the ***Windows NT** Design Workbook*. The
routines are callable from kernel mode within the **Windows NT**
executive. The following routines are presented in subsequent sections:
...
*PoolType* - Supplies the type of pool from which to allocate the bit
map.
...
zone allocator. The remainder of the segment is carved up into fixed
size (BlockSize) blocks and is made available for allocation and
deallocation from the zone. The address of the segment must be aligned
...
*Zone* - Pointer to the zone header controlling the storage from which
...
removing the entry from the allocation list. The lock is released before
...
*Length* (r18) - Supplies the Length, in bytes, of the memory to be
moved.
...
*BaseAddress* - Supplies the base address where the pages were
previously mapped.
...
> The ADDRESS_AND_SIZE_TO_SPAN_PAGES macro takes a virtual address
> and size and returns the number of pages spanned by the size.
...
*TableMutexLevel* - The level number to associated with the mutex that
is used to synchronize access to the handle table.

*SerialNumberMask* - If non-zero then the last 32-bit value in each
handle table entry is supposed to contain a serial number and the value
of this parameter is used to mask off bits that are not part of the
serial number value.
...
*DestroyHandleProcedure* - A pointer to a procedure to call for each
valid handle in the handle table being destroyed.
...
Handle value that the enumeration stopped at. Contents of the variable
...
*Increment* (r17) - Supplies the increment value to be added to the
addend variable.

*Lock* (r18) - Supplies a pointer to a spin lock to be used to
...
*ListEntry* (r17) - Supplies a pointer to the entry to be inserted at
...
*Lock* (r17) - Supplies a pointer to a spin lock to be used to
synchronize access to the list.
...
*String*1 - Pointer to the first string.

*String*2 - Pointer to the second string.
...
*Parameter* - a 32 bit parameter that is stored in $9 simulator
variable.
...
to save the state of the simulator in. If not specified, then the file
name defaults to the image file name with a .CHK extension.
