...
Source: `stuff/docs/resource.doc` (transcribed via soffice + pandoc + cleanup-md.py).
...
- Acquired for exclusive access

- Released (i.e., not acquired for shared or exclusive access)
...
*Wait* - Indicates if the call is allowed to block in order to acquire
the resource. A value of TRUE indicates that the call is allowed to wait
for the resource and FALSE indicates that control is to return
immediately even if the resource cannot be acquired.
...
*Resource* - A pointer to the resource variable to be converted to
shared access
...
however the user calling `ExConvertExclusiveToShared` does not
relinquish access to the resource as the two step operation does.
...
1. Changed `ExChangeSharedToExclusive` to
`ExConvertSharedToExclusive`.
...
1. Changed `ExAcquireResourceShared` and
`ExAcquireResourceExclusive` to take an additional Wait input
parameter and to return a BOOLEAN function value.
