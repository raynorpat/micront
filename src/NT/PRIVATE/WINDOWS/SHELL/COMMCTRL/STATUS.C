...
// MicroNT: Unicode wrapper so Unicode clients (winfile) can create a status
// window.  This comctl32 is built ANSI, so convert the caption and forward to
// the ANSI implementation.
      ...
      HWND hwndParent, UINT wID)
{
    ...
    ach[0] = '\0';
    if (lpszText)
        WideCharToMultiByte(CP_ACP, 0, lpszText, -1, ach, sizeof(ach), NULL, NULL);
    return CreateStatusWindow(style, ach, hwndParent, wID);
}
