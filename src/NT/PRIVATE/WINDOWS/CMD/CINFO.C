...
/* MicroNT: RtlIsTextUnicode is prototyped in <ntrtl.h>, pulled in via
 * cmd.h.  Used below (TYPE builtin) in place of advapi32!IsTextUnicode
 * — see SOURCES for the matching WIN32_LIBS override. */
...
int TyWork(TCHAR *fspec) {
    ...
    do {
        ...
        if (first_read) {
            ...
            /* MicroNT: stock cmd.exe called advapi32!IsTextUnicode here,
             * but that's a one-line wrapper around ntdll!RtlIsTextUnicode
             * (see stuff/.../ADVAPI/ISUNI.C).  Calling Rtl directly drops
             * cmd.exe's only advapi32 import — see SOURCES for the
             * matching WIN32_LIBS override that takes advapi32.lib off
             * the link line. */
            fUnicode = RtlIsTextUnicode((LPWSTR)TypeBuf, bytesread, &result)
                       ? TRUE : FALSE;
