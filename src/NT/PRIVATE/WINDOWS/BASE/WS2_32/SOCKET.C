...

{
    ...
    newSocket->LocalAddress = ALLOCATE_HEAP( sizeof(SOCKADDR_IN) );
    if ( newSocket->LocalAddress == NULL ) {
        error = WSAENOBUFS;
        goto exit;
    }
    newSocket->LocalAddressLength = sizeof(SOCKADDR_IN);

    newSocket->RemoteAddress = ALLOCATE_HEAP( sizeof(SOCKADDR_IN) );
    if ( newSocket->RemoteAddress == NULL ) {
        error = WSAENOBUFS;
        goto exit;
    }
    newSocket->RemoteAddressLength = sizeof(SOCKADDR_IN);
